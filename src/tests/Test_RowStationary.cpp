...
int main() {
    ...
    // int sim_cycles = 0; // count by <cycleMode>

    // Testing Row Stationary mode
    for (int d1 = 0; d1 < DIM1; ++d1) {
        for (int d2 = 0; d2 < DIM2; ++d2) {
            for (int d3 = 0; d3 < DIM3; ++d3) {
                ...
                /*** Row Stationary computation ***/
                Matrix subC = SA.runRowStationary(subA, subB);
                SA.runRowStationary<cycleMode>(subA.rowCount(), subB.colCount());
                ...
                for (int i = 0; i < DIM_SIZE; ++i) 
                    for (int j = 0; j < DIM_SIZE; ++j){
                        if (subC.get(i,j) != subD.get(i,j)){
                            ...
                            cout << "RowStationary:" << subC.get(i,j)<<endl;
                            exit(0);
                        }
                    }
    ...
    // cout << "<CycleMode> Cycles:" << sim_cycles << endl;
    cout << "<CycleMode> Cycles:" << SA.getSimCycles() << endl;
    cout << "RS Tested, Cycles:" << SA.getCycles() << endl;
    return 0;
}
